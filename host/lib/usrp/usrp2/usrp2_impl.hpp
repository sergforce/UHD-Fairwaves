...
public:
    ...
    UHD_INLINE seq_type get_curr_seq_out(void){
        UHD_MSG(fastpath) << "flow_control_monitor::get_curr_seq_out(): " << _last_seq_out << std::endl;
    ...
    UHD_INLINE bool check_fc_condition(double timeout){
        ...
        if (this->ready()) {
            UHD_MSG(fastpath) << "flow_control_monitor::check_fc_condition(): ready() = true\n";
            return true;
        }
        boost::this_thread::disable_interruption di; //disable because the wait can throw
        bool res = _fc_cond.timed_wait(lock, to_time_dur(timeout), _ready_fcn);
        UHD_MSG(fastpath) << "flow_control_monitor::check_fc_condition(): _fc_cond = " << res << std::endl;
        return res;
    }
    ...
    UHD_INLINE void update_fc_condition(seq_type seq){
        boost::mutex::scoped_lock lock(_fc_mutex);
        _last_seq_ack = seq;
        UHD_MSG(fastpath) << "flow_control_monitor::update_fc_condition(): _last_seq_ack = " << _last_seq_ack << std::endl;
        lock.unlock();
        _fc_cond.notify_one();
    }
